...
import { Mail, Lock, User, AlertTriangle, ServerCrash, Loader2 } from "lucide-react";
...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      setError({ message: "Password must be at least 6 characters.", isServerError: false });
      return;
    }
        ...
        <div className="section-container w-full">
          <div className="max-w-md mx-auto">
            ...
            >
              ...
              <p className="text-muted-foreground">Create your GDG developer account</p>
            </motion.div>
                ...
                >
                  ...
                  <div>
                    <p className={`text-sm font-medium ${error.isServerError ? "text-destructive" : "text-foreground"}`}>
                      {error.isServerError ? "Server Connection Error" : "Signup Failed"}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">{error.message}</p>
                    {error.isServerError && (
                      <p className="text-xs text-muted-foreground mt-2 italic">
                        💡 The backend server is not connected yet. Connect your Node.js server to enable registration.
                      </p>
                    )}
                  </div>
                </motion.div>
